...
const CarouselSlide = ({
  ...
  useEffect(() => {
    if ('IntersectionObserver' in window) {
      const io = new IntersectionObserver(
        ([entry]) => setCurrent(entry.intersectionRatio >= 0.75),
  ...
  return (
    <div
      ref={ref}
      className={`${styles.slide} ${isCurrent ? styles.current : ''}`}
    >
      {children}
    </div>
  )
...
const Carousel = ({
  ...
  const onMouseMove = ({ pageX }: MouseEvent) => {
    ...
    if (
      mouse.move &&
      currDX < 0 &&
      currDX > -carousel.current.scrollWidth + carousel.current.offsetWidth
    )
      ...
      >
        <div ref={carousel} style={carouselStyle} className={styles.carousel}>
          {React.Children.map(children, (c, i) => (
            <CarouselSlide key={i} index={i} onCurrent={setCurrent}>
              {c}
            </CarouselSlide>
          ))}
        </div>
      ...
      {dots && (
        <div className={styles.dots}>
          {React.Children.map(children, (_, i) => (
            <div
              key={i}
              className={`${styles.dot} ${current === i ? styles.current : ''}`}
            ></div>
          ))}
        </div>
      )}
