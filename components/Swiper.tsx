...
const Swipable = ({
  ...
  const opacityX =
    100 - Math.abs((mouseState.current.x - mouseState.down.x) / 4)
  const opacityY =
    100 - Math.abs((mouseState.current.y - mouseState.down.y) / 4)
  ...
  return (
    <div
      style={style}
      className={`${styles.swipable} ${moved ? styles.moved : ''} ${
        shake ? styles.shake : ''
      }`}
      {...mouseProps}
    >
...
const Swiper = ({
  ...
  const handleSwipe = () => {
    setCardPositions(
      cardPositions.map((i) => (i === 0 ? cardPositions.length - 1 : i - 1))
    )
  }
  ...
  return (
    <div className={`${styles.swiper} ${className}`} {...mouseProps}>
      {cards.map((content, i) => (
        <Swipable
          shake={shake && i == 0}
          zDistance={zDistance}
          index={cardPositions[i]}
          key={i}
          onSwipe={handleSwipe}
        >
          {content}
